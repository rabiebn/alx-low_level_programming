...
/**
 * idxDigit - find index where a digit is first found in string
 * @s: string to search
...
int _atoi(char *s)
{
	int idxDig = (idxDigit(s));
	int sign;
	int digits_to_print = 0;
	int t = 1, i;
	unsigned int num = 0;
	int digit = (idxDigit(s));

	if (idxDig < 0) /* if no digits found, exit program */
		return (0);
	sign = find_sign(s);

	while ((s[idxDig] >= '0' && s[idxDig] <= '9')
			&& (idxDig <= _strlen(s))) /* count digits to print */
	{
		digits_to_print += 1;
		idxDig++;
	}
